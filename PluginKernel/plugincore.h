...
enum controlID {
	...
	delayTypeOne = 3,
	delayTypeTwo = 13,
	delayOneTwoFeedback = 8,
	delayTwoOneFeedback = 18,
	delayGainThree = 32,
	delayTimeThree = 20,
	delayFeedbackThree = 21,
	delayTimeFour = 40,
	delayFeedbackFour = 41,
	delayModCrossFeedback = 38,
	delayOneFilterType = 4,
	delayOneFilterfc = 5,
	delayTwoFilterType = 14,
	delayTwoFilterfc = 15,
	lfoRate = 51,
	lfoDepth = 52,
	lfoEffectType = 53,
...
public:
	...
	float delayTapOne[2];
	float delayTapTwo[2];
	float delayTapThree[2];
	float delayTapFour[2];
	AudioDelay DelayOne;
	AudioDelay DelayTwo;
	AudioDelay DelayThree;
	AudioDelay DelayFour;
	ProcessFrameInfo pf;
	AudioFilter DelayOnefilter;
	AudioFilter DelayTwofilter;
	AudioFilter DelayThreefilter;
	LFO lfo;
	//AudioFilter DelayFourfilter;
...
private:
	...
	double delayTimeOne = 0.0;
	double delayTimeTwo = 0.0;
	double delayFeedbackOne = 0.0;
	float delayFeedBack2 = 0.f;
	double delayGainOne = 0.0;
	double delayGainTwo = 0.0;
	double dry = 0.0;
	double delayOneTwoFeedback = 0.0;
	double delayTwoOneFeedback = 0.0;
	double delayGainThree = 0.0;
	double delayTimeThree = 0.0;
	float delayFeedbackThree = 0.f;
	double delayTimeFour = 0.0;
	float delayFeedbackFour = 0.f;
	float delayModCrossFeedback = 0.f;
	float delayOneFilterfc = 0.f;
	float delayTwoFilterfc = 0.f;
	double lfoRate = 0.0;
	double lfoDepth = 0.0;
	double delayModFilterQ = 0.0;
	double delayOneFilterQ = 0.0;
	float delayOneFilterDb = 0.f;
	double delayTwoFilterQ = 0.0;
	float delayTwoFilterdB = 0.f;
	float panDelayOne = 0.f;
	float panDelayTwo = 0.f;
	float panDelayMod = 0.f;

	// --- Discrete Plugin Variables 
	int delayTypeOne = 0;
	enum class delayTypeOneEnum { normal,pingpong };	// to compare: if(compareEnumToInt(delayTypeOneEnum::normal, delayTypeOne)) etc... 

	int delayTypeTwo = 0;
	enum class delayTypeTwoEnum { normal,pingpong };	// to compare: if(compareEnumToInt(delayTypeTwoEnum::normal, delayTypeTwo)) etc... 

	int delayOneFilterType = 0;
	enum class delayOneFilterTypeEnum { lpf,hpf,notch,bpf };	// to compare: if(compareEnumToInt(delayOneFilterTypeEnum::lpf, delayOneFilterType)) etc... 

	int delayTwoFilterType = 0;
	enum class delayTwoFilterTypeEnum { lpf,hpf,notch,bpf };	// to compare: if(compareEnumToInt(delayTwoFilterTypeEnum::lpf, delayTwoFilterType)) etc... 

	int lfoEffectType = 0;
	enum class lfoEffectTypeEnum { chorus,flanger,phaser };	// to compare: if(compareEnumToInt(lfoEffectTypeEnum::chorus, lfoEffectType)) etc... 

	int delayOneSolo = 0;
	enum class delayOneSoloEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(delayOneSoloEnum::SWITCH_OFF, delayOneSolo)) etc... 

	int delayTwoSolo = 0;
	enum class delayTwoSoloEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(delayTwoSoloEnum::SWITCH_OFF, delayTwoSolo)) etc... 

	int delayThreeSolo = 0;
	enum class delayThreeSoloEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(delayThreeSoloEnum::SWITCH_OFF, delayThreeSolo)) etc... 

	int modulationSwitch = 0;
	enum class modulationSwitchEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(modulationSwitchEnum::SWITCH_OFF, modulationSwitch)) etc... 
